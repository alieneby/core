...
class BundlingPlugin extends ServerPlugin {
	 ...
	 *
	 * @param  mixed the value encoded in <i>json</i> in appropriate; $bundleMetadata
	 * @throws /Sabre\DAV\Exception\BadRequest
	 * @return array ( $resource )
	 */
	private function storeBundleContents($bundleMetadata) {
		//this is parsing the contents of the files
	...
	private function processBundle($bundleMetadata) {
		...
		while(!$this->contentHandler->getEndDelimiterReached()) {
			...
			if (!isset($bundleContentHeader['content-length']) || !isset($bundleContentHeader['content-id'])) {
			...
			$result = $this->contentHandler->streamReadToStream($target, $bundleContentHeader['content-length']);

			try{
				$node->acquireLock(ILockingProvider::LOCK_SHARED);
				$properties = $node->createFile($target, $fileAttributes);
			} catch (\Exception $e) {
